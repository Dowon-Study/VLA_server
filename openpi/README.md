...
#### Local LIBERO datasets under `/media/idna/Data/Server/Libeoro_dataset`

If your LIBERO dataset already lives on local disk in LeRobot v3 format, you can fine-tune directly on it without pushing anything to the Hub.

This repo now includes local configs for the dataset roots we use in `/media/idna/Data/Server/Libeoro_dataset`:

| Config name | Dataset root under `HF_LEROBOT_HOME` |
| --- | --- |
| `pi05_libero_local_hfvla` | `HuggingFaceVLA_libero_lerobot/` |
| `pi05_libero_local_gt` | `dataset_gt_prepared/` |
| `pi05_libero_local_gt_aug` | `libero_gt_aug_balanced/` |

These configs repack the local LeRobot v3 keys (`observation.images.image`, `observation.images.image2`, `observation.state`, and `action`) into the standard LIBERO policy inputs used by openpi.

First, point `HF_LEROBOT_HOME` at the parent directory containing those dataset roots:

```bash
cd /media/idna/Data/Server/openpi
source scripts/activate_local_env.sh
```

Then make sure the local dataset root has the metadata files expected by the current `lerobot` package:

```bash
.venv/bin/python scripts/prepare_local_lerobot_meta.py \
    --dataset-root /media/idna/Data/Server/Libeoro_dataset/HuggingFaceVLA_libero_lerobot
```

If you still need to prepare the GT-only or GT+aug dataset roots, you can reuse the preparation scripts from the OpenVLA workspace:

```bash
python /media/idna/Data/Server/OpenVLA/scripts/prepare_libero_dataset.py hydrate \
...
python /media/idna/Data/Server/OpenVLA/scripts/prepare_libero_dataset.py merge \
...
Then compute norm stats and start training with one of the local config names:

```bash
uv run scripts/compute_norm_stats.py --config-name pi05_libero_local_gt_aug

XLA_PYTHON_CLIENT_MEM_FRACTION=0.9 uv run scripts/train.py \
    pi05_libero_local_gt_aug \
    --exp-name=my_libero_run \
    --overwrite
```

For PyTorch fine-tuning, convert the JAX base checkpoint first and pass the converted weight directory on the command line:

```bash
uv run examples/convert_jax_model_to_pytorch.py \
    --config_name pi05_libero_local_gt_aug \
    --checkpoint_dir /path/to/jax/base/model \
    --output_path /path/to/pi05_base_pytorch

uv run torchrun --standalone --nnodes=1 --nproc_per_node=2 scripts/train_pytorch.py \
    pi05_libero_local_gt_aug \
    --exp_name=my_libero_run_pt \
